...
/*
function downloadImageNew(string $name, string $link) {
	global $oauth,$headers;
	debug_to_console("inside downloadImageNew [$name] [$link]");
	$name = str_replace("+"," ",urldecode($name));
	$responseImage = $oauth->get($link, $headers);
	debug_to_console("post responseIMAGE TYPE [".gettype($responseImage)."] \n $link"); // gets 202 code response
	$responseImage = $oauth->get($link, $headers);
	debug_to_console("second post responseIMAGE TYPE [".gettype($responseImage)."] \n $link"); // gets real datas
...
*/
			...
			echo "
				<td class='cell'>
					<form action='FieldOperationsHarvestImage.php' method='post' onsubmit='target_popup(this)'>
						<input name='fieldName' value='".urlencode($fieldName)."' hidden>
						<input name='cropName' value='".urlencode($cropName)."' hidden>
						<input name='fieldOperationId' value='".$operationId."' hidden>
						<input name='downloadLink' value='".urlencode($linkFieldOperationImage)."' hidden>
						<button type='submit' name='field-option' value='image' $flagDisabled>IMAGE</button>
					</form>
				</td>";

			//$flagDisabled = checkDisabled($linkShape);
			echo "
				<td class='cell'>
					<form action='FieldOperationsHarvestImage.php' method='post' onsubmit='target_popup(this)'>
						<input name='fieldName' value='".urlencode($fieldName)."' hidden>
						<input name='cropName' value='".urlencode($cropName)."' hidden>
						<input name='fieldOperationId' value='".$operationId."' hidden>
						<input name='downloadLink' value='".urlencode($linkShape)."' hidden>
						<button type='submit' name='field-option' value='image' $flagDisabled>Shapefile</button>
					</form>
				</td>";

			//$flagDisabled = checkDisabled($linkShapeAsync);
			echo "
				<td class='cell'>
					<form action='FieldOperationsHarvestImage.php' method='post' onsubmit='target_popup(this)'>
						<input name='fieldName' value='".urlencode($fieldName)."' hidden>
						<input name='cropName' value='".urlencode($cropName)."' hidden>
						<input name='fieldOperationId' value='".$operationId."' hidden>
						<input name='downloadLink' value='".urlencode($linkShapeAsync)."' hidden>
						<button type='submit' name='field-option' value='image' $flagDisabled>Shapefile Async</button>
					</form>
				</td>";

			//$flagDisabled = checkDisabled($linkMeasurementTypes);
			echo "
				<td class='cell'>
					<form action='FieldOperationsHarvestImage.php' method='post' onsubmit='target_popup(this)'>
						<input name='fieldName' value='".urlencode($fieldName)."' hidden>
						<input name='cropName' value='".urlencode($cropName)."' hidden>
						<input name='fieldOperationId' value='".$operationId."' hidden>
						<input name='downloadLink' value='".urlencode($linkMeasurementTypes)."' hidden>
						<button type='submit' name='field-option' value='image' $flagDisabled>Msrmnt Types</button>
					</form>
				</td>";
			*/

			/*
			echo "<td class='cell'><table><tr>";
				echo "<td>";
					echo "<table><tr>";
						echo "<td>$safra</td>";
